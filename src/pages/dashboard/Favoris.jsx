...
//       // Compter le total (pagination DRF)
//       const count = data?.count ?? (Array.isArray(data) ? data.length : 0);
//       setTotalCount(count);

//       // Extraire le tableau de SavedPath
//       const raw = Array.isArray(data?.results)
//         ? data.results
//         : Array.isArray(data) ? data : [];

//       // ✅ Chaque item est un SavedPath : { id, user, path: {...}, created_at }
//       // On extrait item.path pour avoir le vrai objet Path
//       const paths = raw
//         .map((item) => item?.path || item)
//         .filter((p) => p && p.id);
...
//       // ✅ Tous les chemins sont favoris (ils viennent de la liste favoris)
//       const favoriteIds = new Set(unique.map((p) => Number(p.id)));

//       // Formater avec formatPath pour avoir thumbnail, creator, duration, etc.
//       setFavoris(unique.map((p) => formatPath(p, favoriteIds)));
//       setPage(pageNum);
//     } catch (err) {
//       console.error("Erreur favoris:", err);
...
//     try {
//       await API.delete(`/paths/${id}/favorite/`);
//       // Si la page est vide après suppression, revenir à la page précédente
//       if (favoris.length <= 1 && page > 1) {
//         fetchFavoris(page - 1);
//       }
//     } catch {
//       fetchFavoris(page); // rollback
...
//                         <p className="text-xs font-medium text-[#FEBD00] mt-0.5">{item.establishment}</p>
//                       )}
//                       {/* ✅ departure et destination depuis formatPath */}
//                       {item.departure !== "Départ" && (
//                         <p className="text-xs text-gray-400 mt-0.5 truncate">
//                           📍 {item.departure} → {item.destination}
//                         </p>
