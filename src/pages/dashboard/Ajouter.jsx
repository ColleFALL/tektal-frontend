...
// src/pages/dashboard/Ajouter.jsx
import React, { useEffect, useMemo, useState } from "react";
import { useNavigate } from "react-router-dom";
import {
  ArrowLeft, ChevronDown, Check, X,
  MapPin, Flag, Users, ShieldCheck,
  Video, RefreshCw, Info, Search,
} from "lucide-react";
import API from "../../services/api";

const typesChemin = [
  { key: "community", icon: Users,      label: "Communautaire", desc: "Partagé avec tous" },
  { key: "official",  icon: ShieldCheck, label: "Officiel",       desc: "Validé par un établissement" },
];
...
export default function Ajouter() {
  ...
  // ── Charger les établissements depuis le backend ───────────
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        // Fallback vide — l'utilisateur peut quand même saisir manuellement
        setEstablishments([]);
      } finally {
  ...
  const filteredEstab = useMemo(() => {
    if (!searchEstab.trim()) return establishments;
    return establishments.filter((e) =>
      e.name.toLowerCase().includes(searchEstab.toLowerCase())
    );
  }, [establishments, searchEstab]);

  const validate = () => {
    if (!departure.trim()) {
      alert("Veuillez saisir un point de départ"); return false;
    }
    if (!destination) {
      alert("Veuillez sélectionner une destination"); return false;
    }
    if (departure.trim().toLowerCase() === destination.toLowerCase()) {
      alert("Le départ et la destination doivent être différents"); return false;
    }
    return true;
  };
  ...
  const handleRecord = () => {
    ...
    navigate("/video-upload", {
      state: {
        ...
        destination,
        pathType,
        establishmentId: selectedEstab?.id || null,
      },
    });
  };
    ...
    <div className="min-h-screen w-full bg-gray-200 flex justify-center items-center font-sans">
      <div className="relative w-full max-w-[450px] h-screen bg-white shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="bg-[#FEBD00] px-6 pt-10 pb-7 rounded-b-[35px]">
          ...
          <h1 className="text-3xl font-black text-white">Créer un chemin</h1>
          <p className="mt-2 text-sm text-white/90">Aidez la communauté à se déplacer plus facilement</p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5 pb-28">
          <div className="space-y-6">
            ...
            {/* Départ */}
            <div>
              <label className="mb-2 block text-sm font-bold text-slate-800">Point de départ</label>
              <div className={`flex items-center gap-3 rounded-2xl border px-4 py-4 ${departure.trim() ? "border-green-500 bg-green-50" : "border-slate-200 bg-white"}`}>
                <MapPin size={20} className={departure.trim() ? "text-green-500" : "text-slate-400"} />
                <input
                  type="text"
                  value={departure}
                  onChange={(e) => setDeparture(e.target.value)}
                ...
                />
                {departure && (
                  <button onClick={() => setDeparture("")}><X size={18} className="text-slate-400" /></button>
                )}
              </div>
            </div>
            ...
            {/* Destination */}
            <div>
              <label className="mb-2 block text-sm font-bold text-slate-800">Destination</label>
              <button
                onClick={() => setShowModal(true)}
                className={`flex w-full items-center gap-3 rounded-2xl border px-4 py-4 text-left ${destination ? "border-[#FEBD00] bg-yellow-50" : "border-slate-200 bg-white"}`}
              >
                <Flag size={20} className={destination ? "text-red-500" : "text-slate-400"} />
                <span className={`flex-1 text-sm ${destination ? "text-slate-800 font-medium" : "text-slate-400"}`}>
            ...
            {departure.trim() && destination && (
              <div className="flex items-center gap-3 rounded-2xl border border-yellow-200 bg-yellow-50 px-4 py-3">
                <MapPin size={18} className="text-[#FEBD00]" />
                <p className="text-sm font-semibold text-slate-700">{departure.trim()} → {destination}</p>
              </div>
            )}

            {/* Type chemin */}
            <div>
              <label className="mb-3 block text-sm font-bold text-slate-800">Type de chemin</label>
              <div className="grid grid-cols-2 gap-3">
                {typesChemin.map((type) => {
                  const Icon   = type.icon;
                  const active = pathType === type.key;
                  return (
                    <button
                      key={type.key}
                      onClick={() => setPathType(type.key)}
                      className={`rounded-2xl border-2 p-4 text-center transition ${active ? "border-[#FEBD00] bg-yellow-50" : "border-slate-200 bg-white"}`}
                    >
                      <div className="flex justify-center">
                        <Icon size={28} className={active ? "text-[#FEBD00]" : "text-slate-500"} />
                      </div>
                      <p className={`mt-3 text-sm font-bold ${active ? "text-[#FEBD00]" : "text-slate-800"}`}>{type.label}</p>
                      <p className="mt-1 text-xs leading-5 text-slate-500">{type.desc}</p>
                    </button>
                  );
                })}
              </div>
            ...
            <div className="rounded-2xl border border-yellow-100 bg-white p-5">
              <div className="flex items-center gap-2">
                <Info size={18} className="text-[#FEBD00]" />
                <h2 className="text-base font-black text-slate-900">Comment ça marche ?</h2>
              </div>
              <div className="mt-4 space-y-3">
                {[
                  ...
                  "Ajoutez les étapes du trajet",
                ].map((item, i) => (
                  <div key={i} className="flex items-start gap-3">
                    <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[#FEBD00] text-xs font-bold text-white">{i + 1}</div>
                    <p className="pt-1 text-sm leading-6 text-slate-600">{item}</p>
                  </div>
                ))}
              </div>
            ...
            <button
              onClick={handleRecord}
              className="flex w-full items-center justify-center gap-3 rounded-2xl bg-[#FEBD00] px-5 py-4 text-base font-bold text-white shadow-lg active:scale-[0.98]"
            >
              <Video size={22} /> Importer une vidéo
            </button>
          </div>
        </div>
        ...
        {showModal && (
          <div className="absolute inset-0 z-30 flex items-end bg-black/50">
            <div className="w-full rounded-t-[28px] bg-white max-h-[75%] flex flex-col overflow-hidden">
              <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
                <h3 className="text-lg font-black text-slate-900">Choisir la destination</h3>
                <button onClick={() => { setShowModal(false); setSearchEstab(""); }} className="h-9 w-9 flex items-center justify-center rounded-full bg-slate-100">
                  <X size={18} className="text-slate-700" />
                </button>
              </div>
              ...
              <div className="overflow-y-auto px-4 py-2 flex-1">
                {loadingEstab ? (
                  ...
                  <div className="py-12 text-center">
                    <Flag size={36} className="mx-auto text-slate-300" />
                    <p className="mt-4 text-sm text-slate-500">Aucun établissement trouvé</p>
                  </div>
                ) : (
                  <div className="space-y-2 pb-4">
                    {filteredEstab.map((item) => {
                      const selected = selectedEstab?.id === item.id;
                      return (
                        <button
                          key={item.id}
                          ...
                          className={`flex w-full items-center gap-3 rounded-2xl px-4 py-4 text-left ${selected ? "bg-yellow-50" : "bg-white hover:bg-slate-50"}`}
                        >
                          <div className="shrink-0">
                            {selected
                              ? <Check size={20} className="text-[#FEBD00]" />
                          ...
                          <div className="flex-1">
                            <p className={`text-sm ${selected ? "font-bold text-[#FEBD00]" : "font-medium text-slate-800"}`}>{item.name}</p>
                            {item.address && <p className="mt-0.5 text-xs text-slate-400">{item.address}</p>}
                          </div>
                        </button>
                      );
                    })}
