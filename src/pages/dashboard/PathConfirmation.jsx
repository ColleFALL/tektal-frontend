  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      const stepsWithTime = steps?.map((s, i) => ({
        ...
        end_time:    i === stepCount - 1 ? totalDuration : (i + 1) * timePerStep,
      })) || [];
      ...
      if (data) {
        ...
        else msg = Object.entries(data).map(([key, val]) => `${key}: ${Array.isArray(val) ? val.join(", ") : val}`).join(" | ");
      }
      setError(msg);
    } finally {
      setSubmitting(false);
  ...
  if (createdPath) {
    return (
      <div className="min-h-screen w-full bg-gray-200 flex justify-center items-center font-sans">
        <div className="relative w-full max-w-[450px] h-screen bg-white shadow-2xl overflow-hidden flex flex-col">
          <div className="flex-1 px-6 py-10 flex flex-col items-center justify-center text-center pb-28">
            ...
            <div className="mt-8 w-full space-y-3">
              ...
              {/* ✅ Bouton partage ouvre ShareModal */}
              <button
        ...
        {/* ✅ ShareModal */}
        {showShare && <ShareModal path={createdPath} onClose={() => setShowShare(false)} />}
      </div>
    );
  }
    ...
    <div className="min-h-screen w-full bg-gray-200 flex justify-center items-center font-sans">
      <div className="relative w-full max-w-[450px] h-screen bg-white shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5 pb-28 space-y-4">
          ...
          >
            {submitting ? (
              <><Loader2 size={20} className="animate-spin" />{progress || "Publication..."}</>
            ) : (
