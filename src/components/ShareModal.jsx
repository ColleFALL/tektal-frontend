...
export default function ShareModal({ path, onClose }) {
  ...
  const handleNativeShare = async () => {
    ...
    try {
      await navigator.share({ title: path?.title, text: shareText, url: shareUrl });
    } catch { /* ignoré */ }
  };
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h3 className="font-black text-slate-900 text-lg">Partager</h3>
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          ...
          <button onClick={handleWhatsApp} className="flex flex-col items-center gap-2">
            <div className="h-14 w-14 rounded-2xl flex items-center justify-center" style={{ backgroundColor: "#25D366" }}>
          ...
          <button onClick={handleFacebook} className="flex flex-col items-center gap-2">
            <div className="h-14 w-14 rounded-2xl flex items-center justify-center" style={{ backgroundColor: "#1877F2" }}>
              <svg viewBox="0 0 24 24" className="w-7 h-7 fill-white">
          ...
          {navigator.share && (
            <button onClick={handleNativeShare} className="flex flex-col items-center gap-2">
              <div className="h-14 w-14 rounded-2xl bg-[#FEBD00] flex items-center justify-center">
                <svg viewBox="0 0 24 24" className="w-6 h-6 fill-white">
                  <path d="M18 16.08c-.76 0-1.44.3-1.96.77L8.91 12.7c.05-.23.09-.46.09-.7s-.04-.47-.09-.7l7.05-4.11c.54.5 1.25.81 2.04.81 1.66 0 3-1.34 3-3s-1.34-3-3-3-3 1.34-3 3c0 .24.04.47.09.7L8.04 9.81C7.5 9.31 6.79 9 6 9c-1.66 0-3 1.34-3 3s1.34 3 3 3c.79 0 1.5-.31 2.04-.81l7.12 4.16c-.05.21-.08.43-.08.65 0 1.61 1.31 2.92 2.92 2.92s2.92-1.31 2.92-2.92-1.31-2.92-2.92-2.92z"/>
                </svg>
              </div>
              <span className="text-xs font-medium text-gray-600">Autres</span>
            </button>
          )}
        </div>
        ...
        <div className="flex items-center gap-3 bg-gray-50 rounded-2xl px-4 py-3">
          ...
          <button
            onClick={handleCopy}
            className="flex items-center gap-1.5 bg-[#FEBD00] text-white text-xs font-bold px-4 py-2 rounded-full flex-shrink-0"
          >
